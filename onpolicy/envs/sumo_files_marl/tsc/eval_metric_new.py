...
# from tsc.eval_config import map_configs, name, metrics #从evalconfig文件中读取需要评测的地图和实验信息
from CoEMV_origin.onpolicy.envs.sumo_files_marl.tsc.eval_config import map_configs, name, metrics #从evalconfig文件中读取需要评测的地图和实验信息
...
path_root = './envs/'
log_dir = path_root + 'sumo_files_marl/sumo_logs/'
env_base = path_root + 'sumo_files_marl/scenarios/'

# trials = [folder for folder in next(os.walk(log_dir + f"eval_{name}" + os.sep))[1]]#重复实验对应的文件夹
# trials = ['arterial4x4', 'nanshan', 'grid4x4', 'fenglin', 'cologne8', 'ingolstadt21']
trials = ['sumo_fenglin_base_road', 'grid4x4', 'arterial4x4', 'ingolstadt21', 'cologne8']

for metric in metrics:
    output_file = log_dir + f"{name}" + os.sep + 'avg_{}.py'.format(metric)
    run_avg = dict()
    ...
    for map_name in map_configs:#地图名
        ind_ += 1
        average_per_episode = []
        for i in range(2, 200, 2):#偶数结尾的文件才有数据
        # for i in range(2, 588, 2):#偶数结尾的文件才有数据
            #trip_file_name = log_dir + names + os.sep + names + "_" + map_name + ".sumocfg_{}_trip.xml".format(i)
            # trip_file_name = log_dir + f"{name}" + os.sep + trial + os.sep + name + "_" + map_name + ".sumocfg_{}_trip.xml".format(i)
            trip_file_name = log_dir + f"{name}" + os.sep + trials[ind_] +"/trial_685/test_gat_" + map_name + ".sumocfg_{}_trip.xml".format(i)#log_dir+name + os.sep + 'tripinfo_'+str(i)+'.xml'
            ...
            try:
                ...
                if metric == 'timeLoss' and map_name in ['cologne8', 'ingolstadt21', 'base', 'rl_wj','arterial4x4']:    # Calc. departure delays
                    #只有具体的每辆车depart的时间的地图才能计算，其他地图车流随机生成，没有固定的depart时间
                    #对delay的计算影响不是很大
                    
                    #读取route文件获取预置的车流信息
                    route_file_name = path_root + map_configs[map_name]['route']
