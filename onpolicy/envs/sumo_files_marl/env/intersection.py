...
class Intersection:
    def __init__(self, tl_id, msg, env,
        ...
        self._id = tl_id
        self._env = env
        self.state = state  #解析net.xml获得的交通信息
        ...
        try:
            ...
            self.tl_ava = False #表示信号灯是否有效
            return
        # seq = ["r", "s", "l"]  right always can go though
        self.seq = ["l", "s"]   #信号灯的动作序列，例如"左转"、"直行"等

        self.lans_link = self._env.sim.trafficlight.getControlledLinks(self._id)
        self._incominglanes_links = {}  # from : to: [traffic light index, direction]进入车道与信号灯之间的映射关系
        try:
        ...
        # print(self._incominglanes_links, "incoming lanes")
        self.entering_approaches2phase = {} #进入方向到信号灯阶段的映射关系
        # print(self._incominglanes_links) {'A1A0_0': {'A0left0_0': [0, 'r'], 'A0left0_1': [1, 'r'], 'A0left0_2': [2, 'r']}, 'A1A0_1': {'A0bottom0_0': [3, 's'], 'A0bottom0_1': [4, 's'], 'A0bottom0_2': [5, 's']}...
        # print(self.lans_link)[[('A1A0_0', 'A0left0_0', ':A0_0_0')], [('A1A0_0', 'A0left0_1', ':A0_0_1')], [('A1A0_0', 'A0left0_2', ':A0_0_2')], [('A1A0_1', 'A0bottom0_0', ':A0_3_0')], ...
        ...
        # print(self.entering_approaches2phase){'bottom0A0': {'l': [['bottom0A0_2', (24, 25, 26)]], 'r': [['bottom0A0_0', (18, 19, 20)]], 's': [['bottom0A0_1', (21, 22, 23)]]}, 'A1A0': {'l': [['A1A0_2', (6, 7, 8)]], 'r': [['A1A0_0', (0, 1, 2)]], 's': [['A1A0_1', (3, 4, 5)]]}, 'left0A0': {'s': [['left0A0_1', (30, 31, 32)]], 'r': [['left0A0_0', (27, 28, 29)]], 'l': [['left0A0_2', (33, 34, 35)]]}, 'B0A0': {'l': [['B0A0_2', (15, 16, 17)]], 's': [['B0A0_1', (12, 13, 14)]], 'r': [['B0A0_0', (9, 10, 11)]]}}
        # print(self._incoming_lanes)['bottom0A0_2', 'A1A0_2', 'left0A0_1', 'bottom0A0_0', 'B0A0_2', 'A1A0_0', 'left0A0_0', 'B0A0_1', 'A1A0_1', 'bottom0A0_1', 'B0A0_0', 'left0A0_2']
        self.outgoing_sequence = list("_".join(i.split("_")[:-1]) for i in msg['leaving_lanes'])
        # print(self.entering_sequence,"entering_sequence")
        # print(self.outgoing_sequence,"outgoing_sequence")
        # print(self.entering_approaches2phase,"entering_approaches2phase")
        ...
        if self.tl_ava:
            ...
            if not_default:
                ...
                # print(self.unava_index, "unava_index")
                signal_definition = self._env.sim.trafficlight.getAllProgramLogics(self._id)[-1]
                # print(signal_definition, "signal_definition")
    ...
    def get_vehicles_in_range(self):
        """
        获取所有进入该信号灯控制范围内的车辆，并为每个车辆创建代理。
        """
        self.vehicles_in_range=[]  # 清空之前的代理
        for lane in self._incoming_lanes:
            # 获取该车道上的所有车辆ID
            # print(lane)

            vehicles_in_lane = self._env.sim.lane.getLastStepVehicleIDs(lane)
            # print(self._env.sim.lane.getLastStepHaltingNumber(lane))
            for vehicle_id in vehicles_in_lane:
                self.vehicles_in_range.append(vehicle_id)
                # distance = self._env.sim.vehicle.getNextTLS(vehicle_id)
                #
                # # 如果车辆距离信号灯小于控制范围，则认为进入了信号灯控制范围
                # if distance[0][2] <= 200:
                #     self.vehicles_in_range.append(vehicle_id)
    ...
    def reset_tl_logic(self):
        ...
        logic = []
        unava_index = []
        right_index = []    #[0, 1, 2, 9, 10, 11, 18, 19, 20, 27, 28, 29]
        for i in range(len(self.lans_link)):
            # debug
            if i not in np.concatenate(self.phase_index):
        ...
        index2dirc = {} #{0: 'r', 1: 'r', 2: 'r', 3: 's', 4: 's', 5: 's', 6: 'l', 7: 'l', 8: 'l', 9: 'r', 10: 'r', 11: 'r', 12: 's', 13: 's', 14: 's', 15: 'l', 16: 'l', 17: 'l', 18: 'r', 19: 'r', 20: 'r', 21: 's', 22: 's', 23: 's', 24: 'l', 25: 'l', 26: 'l', 27: 'r', 28: 'r', 29: 'r', 30: 's', 31: 's', 32: 's', 33: 'l', 34: 'l', 35: 'l'}
    ...
    def get_lane_wait_time_emv(self):
        wait_time_dict = {}
        for lane in self._incoming_lanes:
            wait_time = 0
            for veh in self._lane_vehicle_dict[lane]:
                if self._env.sim.vehicle.getTypeID(veh) =='emergency':
                    cur_wait_time = self._env.sim.vehicle.getAccumulatedWaitingTime(veh)
                    if 'wait_time' not in self._env.vehicle_info[veh]:
                        self._env.vehicle_info[veh]['wait_time'] = 0
                    wait_time += cur_wait_time - self._env.vehicle_info[veh]['wait_time']
    ...
    def get_lane_delay_time_emv(self):
        delay_time_dict = {}
        for lane in self._incoming_lanes:
            delay_time = 0
            for veh in self._lane_vehicle_dict[lane]:
                # 获取车辆类型
                vehicle_type = self._env.sim.vehicle.getTypeID(veh)

                # 仅计算紧急车辆的延误时间
                if vehicle_type != "emergency":
                    continue

                cur_distance = self._env.sim.vehicle.getDistance(veh)
                cur_time = self._env.sim.vehicle.getLastActionTime(veh)

                veh_info = self._env.vehicle_info.setdefault(veh, {'distance': cur_distance, 'time': cur_time})

                real_distance = cur_distance - veh_info['distance']
                target_speed = self._env.sim.vehicle.getMaxSpeed(veh)
                target_distance = (cur_time - veh_info['time']) * target_speed

                veh_info['distance'], veh_info['time'] = cur_distance, cur_time

                if target_speed > 0:
                    delay_time += (target_distance - real_distance) / target_speed
    ...
    def get_reward(self, reward_type, step_tl):
        ...
        if (step_tl)%15 == 0:
            ...
            if 'wait_time_emv' in reward_type:
                wait_time_emv = np.average(list(self.get_lane_wait_time_emv().values())) / 1000
                reward['wait_time'] = -wait_time_emv
            if 'delay_time_emv' in reward_type:
                delay_time_emv = np.average(list(self.get_lane_delay_time_emv().values())) / 1e4
                reward['delay_time'] = -delay_time_emv
        else:
            if 'queue_len' in reward_type:
                queue_len = 0
                reward['queue_len'] = -queue_len
            ...
            if 'delay_time_emv' in reward_type:
                delay_time_emv = 0
                reward['delay_time_emv'] = -delay_time_emv
        return reward

    def get_state(self, step_tl):
