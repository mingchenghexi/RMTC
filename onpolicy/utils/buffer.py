...
from CoEMV_graph.onpolicy.utils.util import get_shape_from_obs_space, get_shape_from_act_space
...
class SharedReplayBuffer(object):
    ...
    def __init__(self, args, tl_num_agents,ve_num_agents,emv_num_agents, tl_obs_space,ve_obs_space,emv_obs_space, cent_obs_space, tl_act_space, ve_act_space, emv_act_space):
        self.args = args
        ...
        self.tl_num_agents = tl_num_agents
        self.ve_num_agents = ve_num_agents
        self.emv_num_agents = emv_num_agents

        self.ve_obs_shape = get_shape_from_obs_space(ve_obs_space)
        self.emv_obs_shape = get_shape_from_obs_space(emv_obs_space)
        self.tl_obs_shape = get_shape_from_obs_space(tl_obs_space)
        ...
        self.tl_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, tl_num_agents, *self.tl_obs_shape),
                                  dtype=np.float32)

        self.ve_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, ve_num_agents, *self.ve_obs_shape), dtype=np.float32)
        self.emv_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, emv_num_agents, *self.emv_obs_shape),
                            dtype=np.float32)

        self.tl_rnn_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, tl_num_agents, self.recurrent_N, self.hidden_size),
            dtype=np.float32)
        self.ve_rnn_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, ve_num_agents, self.recurrent_N, self.hidden_size),
            dtype=np.float32)
        self.emv_rnn_states = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, emv_num_agents, self.recurrent_N, self.hidden_size),
            dtype=np.float32)

        self.tl_rnn_states_critic = np.zeros_like(self.tl_rnn_states)
        self.ve_rnn_states_critic = np.zeros_like(self.ve_rnn_states)
        self.emv_rnn_states_critic = np.zeros_like(self.emv_rnn_states)

        self.tl_value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, tl_num_agents, 1), dtype=np.float32)
        self.ve_value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, ve_num_agents, 1), dtype=np.float32)
        self.emv_value_preds = np.zeros(
            (self.episode_length + 1, self.n_rollout_threads, emv_num_agents, 1), dtype=np.float32)

        self.tl_returns = np.zeros_like(self.tl_value_preds)
        self.ve_returns = np.zeros_like(self.ve_value_preds)
        self.emv_returns = np.zeros_like(self.emv_value_preds)

        self.tl_advantages = np.zeros(
            (self.episode_length, self.n_rollout_threads, tl_num_agents, 1), dtype=np.float32)
        self.ve_advantages = np.zeros(
            (self.episode_length, self.n_rollout_threads, ve_num_agents, 1), dtype=np.float32)
        self.emv_advantages = np.zeros(
            (self.episode_length, self.n_rollout_threads, emv_num_agents, 1), dtype=np.float32)

        self.tl_act_space = tl_act_space
        self.ve_act_space = ve_act_space
        self.emv_act_space = emv_act_space

        if tl_act_space.__class__.__name__ == 'Discrete':
            self.tl_available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, tl_num_agents, tl_act_space.n),
                                             dtype=np.float32)
            self.ve_available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, ve_num_agents, ve_act_space.n),
                                             dtype=np.float32)
            self.emv_available_actions = np.ones((self.episode_length + 1, self.n_rollout_threads, emv_num_agents, emv_act_space.n),
                                             dtype=np.float32)
        else:
            self.tl_available_actions = None
            self.ve_available_actions = None
            self.emv_available_actions = None


        tl_act_shape = get_shape_from_act_space(tl_act_space)
        ve_act_shape = get_shape_from_act_space(ve_act_space)
        emv_act_shape = get_shape_from_act_space(emv_act_space)

        self.tl_actions = np.zeros(
            (self.episode_length, self.n_rollout_threads, tl_num_agents, tl_act_shape), dtype=np.float32)
        self.tl_action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, tl_num_agents, tl_act_shape), dtype=np.float32)
        self.tl_rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, tl_num_agents, 1), dtype=np.float32)

        self.ve_actions = np.zeros(
            (self.episode_length, self.n_rollout_threads, ve_num_agents, ve_act_shape), dtype=np.float32)
        self.ve_action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, ve_num_agents, ve_act_shape), dtype=np.float32)
        self.ve_rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, ve_num_agents, 1), dtype=np.float32)

        self.emv_actions = np.zeros(
            (self.episode_length, self.n_rollout_threads, emv_num_agents, emv_act_shape), dtype=np.float32)
        self.emv_action_log_probs = np.zeros(
            (self.episode_length, self.n_rollout_threads, emv_num_agents, emv_act_shape), dtype=np.float32)
        self.emv_rewards = np.zeros(
            (self.episode_length, self.n_rollout_threads, emv_num_agents, 1), dtype=np.float32)
        ...
        self.graph_obs = np.zeros((self.episode_length + 1, self.n_rollout_threads, num_agents, *self.obs_shape),
                            dtype=np.float32)
    ...
    def insert_0(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
        ...
        self.rnn_states[self.step] = rnn_states_actor.copy()
        self.rnn_states_critic[self.step] = rnn_states_critic.copy()
        self.masks[self.step] = masks.copy()
        # for i in range(self.num_agents):
        #     self.graph[self.step][0][i] = copy.deepcopy(graph)
        self.graph_obs[self.step] = graph_obs.copy()
    ...
    def insert_ve_0(self, share_obs, obs, rnn_states_actor, rnn_states_critic, actions, action_log_probs,
        ...
        for i, r in enumerate(return_ve[0]):
            if r[0] == True:
                ...
                self.graph_obs[self.step_ve[i] - 1][0][i] = graph_obs[i].copy()
                ...
                if self.step_ve[i] > 1:
                    ...
                    self.rnn_states[self.step_ve[i]-1][0][i]= rnn_states_actor[:,i].copy()
                    self.rnn_states_critic[self.step_ve[i]-1][0][i] = rnn_states_critic[:,i].copy()
    ...
    def recurrent_generator(self, advantages, num_mini_batch, data_chunk_length):
        ...
        graph = _cast(self.graph_obs[:self.step])
    ...
    def recurrent_generator_ve(self, advantages, num_mini_batch, data_chunk_length):
        ...
        if len(self.share_obs.shape) > 4:
            ...
            share_obs_list, obs_list, next_obs_list, graph_list = [], [], [], []
            ...
            for i in range(num_agents):
                ...
                if step > 0:  # 只有当步数大于0时才处理数据
                    ...
                    graph_list.append(_cast(self.graph_obs[:step, :, i, :][:, :, np.newaxis, :]))
            ...
            graph = np.concatenate(graph_list, axis=0)
