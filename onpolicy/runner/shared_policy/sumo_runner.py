...
import numpy as np
import torch
from onpolicy.runner.shared_policy.base_runner import Runner
import wandb
import imageio
import copy
import sys
...
class SUMORunner(Runner):
    ...
    def run(self):
        ...
        for episode in range(episodes):
            ...
            for step in range(10000000):
                ...
                obs_tl, rewards_tl, dones_tl, return_ve, infos_tl, obs_ve, rewards_ve, dones_ve, infos_ve, graph_ve, obs_change_ve, new_ves, new_emv, emv, ve, travel_info  = self.envs.step(actions_tl.astype(np.int64)[:, :, 0], actions_ve.astype(np.int64)[:, :, 0])
                ...
                if episode %25 == 0:
                    graph_tl = self.get_graph_state(graph_ve)
                    if step % 30 == 0 and step != 0:
                        graph_tl_last = self.get_graph_state(self.graph)
                        self.train_gnn(graph_tl, obs_tl, obs_vehicle, obs_emv, graph_tl_last)
                else:
                    with torch.no_grad():
                        graph_tl = self.get_graph_state(graph_ve)
                self.graph = graph_ve
                intrinsic_reward_tl = self.compute_reward(graph_tl['signal_light'],graph_tl['emergency'], _n2t(rewards_emv, self.device).squeeze(0))
                intrinsic_reward_ve = self.compute_reward(graph_tl['vehicle'], graph_tl['emergency'], _n2t(rewards_emv, self.device).squeeze(0))
                rewards_vehicle = rewards_vehicle + 1*intrinsic_reward_ve
                rewards_ve = np.concatenate((rewards_vehicle, rewards_emv), axis=1)
                rewards_tl = rewards_tl + 0.01*intrinsic_reward_tl
                if (step+1) % 15 == 0:
                ...
                data_ve = obs_ve, rewards_ve, dones_ve, infos_ve, values_ve, actions_ve, action_log_probs_ve, rnn_states_ve, rnn_states_critic_ve, available_actions_ve, return_ve, obs_change_ve, graph_tl
            ...
            if episode % 25 !=  0:
                ...
                if episode % self.log_interval == 0:
                    ...
                    self.log_train_tl(train_infos, episode)
                    self.log_train_vehicle(train_infos_ve, episode)

                    self.log_env_tl(env_infos_tl, episode)
                    self.log_env_vehicle(env_infos_ve, episode)
                    self.log_env_emv(env_infos_emv, episode)
                    self.log_travel_train(travel_info, episode)
                # self.log
    ...
    def train_gnn(self, graph_tl, obs_tl, obs_ve, obs_emv, graph_tl_last):
        ...
        self.trainer.train_gnn(graph_tl, rnn_state_tl, rnn_state_ve, rnn_state_emv, graph_tl_last)
        # self.trainer_ve.train_gnn_emv(graph_ve, rnn_state_ve, rnn_state_emv, graph_vehicle_last)
    ...
    # def get_ava_actions(self, ava): #----修改上面的原本的代码
    #     available_actions = np.ones(
    #         (self.all_args.n_rollout_threads, self.all_args.num_agents, self.all_args.num_actions))

    #     if isinstance(ava, list):
    #         # 判断是否是二维 list，形如 [[3, 7], [3, 4, 5], ...]
    #         if all(isinstance(row, list) for row in ava) and \
    #         all(isinstance(elem, int) for row in ava for elem in row):
    #             # case 1: shape == (n_threads, n_agents)，每个元素是动作 index（int）
    #             for j in range(self.all_args.n_rollout_threads):
    #                 for i in range(self.all_args.num_agents):
    #                     # 防止越界
    #                     if j < len(ava) and i < len(ava[j]):
    #                         action = ava[j][i]
    #                         if isinstance(action, int) and action < self.all_args.num_actions:
    #                             available_actions[j, i, action] = 0

    #         elif all(isinstance(row, list) for row in ava) and \
    #             all(isinstance(elem, list) and len(elem) > 0 for row in ava for elem in row):
    #             # case 2: shape == (n_threads, n_agents, ?)，每个元素是 list，取第一个
    #             for j in range(self.all_args.n_rollout_threads):
    #                 for i in range(self.all_args.num_agents):
    #                     if j < len(ava) and i < len(ava[j]):
    #                         action_list = ava[j][i]
    ...
    #                                 if isinstance(a, int) and a < self.all_args.num_actions:
    #                                     available_actions[j, i, a] = 0
    ...
    def eval(self, total_num_steps):
        ...
        eval_obs_ve = np.zeros((self.n_rollout_threads, *self.ve_buffer.obs.shape[2:]), dtype=np.float32)
        eval_rnn_states_ve = np.zeros((self.n_rollout_threads, *self.ve_buffer.rnn_states.shape[2:]),dtype=np.float32)
        eval_masks_ve = np.ones((self.n_rollout_threads, self.num_ve_agents, 1), dtype=np.float32)
        eval_graph_ve = np.zeros(self.ve_buffer.graph_obs.shape[2:], dtype=np.float32)

        available_actions_ve = np.ones(
            (self.all_args.n_rollout_threads, self.all_args.num_ve_agents, self.all_args.num_ve_actions))
        ...
        for eval_step in range(10000000):
            ...
            eval_action_tl, rnn_states_tl = self.trainer.policy.act(np.concatenate(eval_obs_tl),
                                                ...
                                                deterministic=True)
            eval_action_ve, rnn_states_ve = self.trainer_ve.policy.act(np.concatenate(eval_obs_ve),
                                                                   np.concatenate(eval_rnn_states_ve),
                                                                   np.concatenate(eval_masks_ve),
                                                                    eval_graph_ve,
                                                                    available_actions=available_actions_ve.squeeze(0),
                                                                   deterministic=True)
            ...
            eval_actions_ve = np.array(np.split(_t2n(eval_action_ve), self.n_eval_rollout_threads))
            rnn_states_ve = np.array(np.split(_t2n(rnn_states_ve), self.n_eval_rollout_threads))
            # Obser reward and next obs
            # eval_obs_tl, eval_rewards, eval_dones, eval_infos = self.eval_envs.step(eval_actions_env)
            obs_tl, eval_rewards_tl, eval_dones_tl, eval_return_ve, eval_infos_tl, eval_obs_ve, eval_rewards_ve, eval_dones_ve, eval_infos_ve, eval_graph, eval_obs_change_ve, eval_new_ves, eval_new_emv, eval_ves, eval_emv, travel_info = self.eval_envs.step(eval_actions_tl.astype(np.int64)[:, :, 0], eval_actions_ve.astype(np.int64)[:, :, 0])
            ...
            with torch.no_grad():
                graph_ve = self.get_graph_state(eval_graph)
                graph_tl = _t2n(graph_ve['signal_light'].to(torch.float))
            ...
            eval_rnn_states_ve[eval_dones_ve == True] = np.zeros(
                ((eval_dones_ve == True).sum(), self.recurrent_N, self.hidden_size), dtype=np.float32)
            eval_masks_ve = np.ones((self.n_eval_rollout_threads, self.num_ve_agents, 1), dtype=np.float32)
            eval_masks_ve[eval_dones_ve == True] = np.zeros(((eval_dones_ve == True).sum(), 1),
                                                                      dtype=np.float32)


            for i, r in enumerate(eval_obs_change_ve[0]):
                if r[0] == True:
                    eval_graph_ve[i, :] = graph_ve[i, :]
                if eval_return_ve[0][i] == True:
                    eval_rnn_states_ve[:, i, :] = rnn_states_ve[:, i, :]
            ...
            if eval_step == 3599:
                self.log_travel_eval_3600(travel_info, total_num_steps)
        ...
        self.log_eval_info_tl(eval_infos_tl, total_num_steps)
        self.log_eval_info_ve(eval_infos_ve, total_num_steps)
        self.log_travel_eval(travel_info, total_num_steps)
